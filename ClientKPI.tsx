...
import { UsersIcon, LightbulbIcon, MessageSquareIcon, PhoneIncomingIcon, PlusIcon, ChevronLeftIcon, ChevronRightIcon } from '../constants';
...
const CHANNEL_COLORS: Record<ContactChannel, string> = { [ContactChannel.INSTAGRAM]:'#c13584', [ContactChannel.WHATSAPP]:'#25D366', [ContactChannel.WEBSITE]:'#3b82f6', [ContactChannel.PHONE]:'#64748b', [ContactChannel.REFERRAL]:'#f97316', [ContactChannel.OTHER]:'#9ca3af'};
...
const ClientKPI: React.FC<ClientKPIProps> = ({ clients, setClients, projects, leads, setLeads, showNotification }) => {
    ...
    const { topLocations, channelDistribution } = useMemo(() => {
        ...
        const channelDistribution = (Object.entries(channels) as [ContactChannel, number][]).map(([label, value]) => ({label, value, color: CHANNEL_COLORS[label]}));
    ...
    const handleOpenUpdateLeadModal = (lead: Lead) => { setSelectedLead(lead); setNewLeadStatus(lead.status); setIsUpdateLeadModalOpen(true); };
    const handleCloseUpdateLeadModal = () => setIsUpdateLeadModalOpen(false);
    const handleUpdateLead = () => {
        if (!selectedLead) return;
        
        if (newLeadStatus === LeadStatus.CONVERTED) {
            ...
            showNotification(`Prospek ${selectedLead.name} berhasil dikonversi menjadi klien baru!`);
        } else {
            setLeads(prev => prev.map(l => l.id === selectedLead.id ? { ...l, status: newLeadStatus } : l));
        }
    ...
    return (
        <div className="space-y-8">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm">
                    ...
                    <div className="overflow-x-auto"><table className="w-full text-sm text-left text-slate-500">
                        <thead className="text-xs text-slate-700 uppercase bg-slate-50"><tr>
                            <th className="px-4 py-3">Nama Prospek</th><th className="px-4 py-3">Kanal</th><th className="px-4 py-3">Lokasi</th><th className="px-4 py-3">Status Chat</th><th className="px-4 py-3 text-center">Aksi</th>
                        </tr></thead>
                        <tbody className="divide-y divide-slate-200">
                            {filteredLeads.map(lead => (<tr key={lead.id}>
                                ...
                                <td className="px-4 py-3 text-center"><button onClick={() => handleOpenUpdateLeadModal(lead)} className="button-secondary text-xs px-3 py-1">Update Status</button></td>
                            </tr>))}
            ...
            <Modal isOpen={isUpdateLeadModalOpen} onClose={handleCloseUpdateLeadModal} title={`Update Status Prospek: ${selectedLead?.name}`}>
                {selectedLead && <div className="space-y-4">
                    <p className="text-sm">Pilih status baru untuk prospek ini.</p>
                    <div><label className="block text-sm font-medium text-slate-700 mb-1">Status Chat Baru</label><select value={newLeadStatus} onChange={(e) => setNewLeadStatus(e.target.value as LeadStatus)} className="input-field w-full">
                        {Object.values(LeadStatus).map(s => <option key={s} value={s}>{s}</option>)}
                    </select></div>
                    {newLeadStatus === LeadStatus.CONVERTED && <p className="text-xs text-emerald-600 bg-emerald-50 p-2 rounded-md">Menyimpan akan memindahkan prospek ini ke daftar klien Anda.</p>}
                    <div className="pt-4 flex justify-end gap-2"><button type="button" onClick={handleCloseUpdateLeadModal} className="button-secondary">Batal</button><button type="button" onClick={handleUpdateLead} className="button-primary">Simpan Status</button></div>
                </div>}
            </Modal>
